...
def prep_harmonie(input,grid):
  variables = ['ua','va','w','ta','hus','clw','ps','tas','huss']
  if('synturb' in input): variables = variables+['tke','tauu','tauv','hfss'] # cb removed
  data = []
  # Open data and crop data
  for var in variables:
    print(f"trying to open {input['inpath']}{var}.*.nc")
    with xr.open_mfdataset(f"{input['inpath']}{var}.*.nc",chunks={"time": input['tchunk']}) as ds:
      # Get time epochs
      if(var==variables[0]):
        ...
        time = ds['time'].values
        dts = (time[1]-time[0])/np.timedelta64(1, 's')
      # Interpolate fluxes to same time
      if(var in ['tauu','tauv','hfss']):
        ds = ds.diff('time') # deaccumulate
        ds[var] = ds[var]/dts
  ...
  variables = ['uas','vas','ws','clws']
  if('synturb' in input): variables.append('tkes')
  data = data.assign({var:xr.zeros_like(data['ps']) for var in variables})
  if('synturb' in input):
    tauu = data['tauu']
    tauv = data['tauv']
    hfss = data['hfss'] # NOTE this is an accumulated flux 
    #cb   = data['cb']
  # Concatenate surface and 3D fields
  variables = ['ua','va','w','ta','hus','clw','p']
 #  if('synturb' in input): variables.append('tke') # FJ removing TKE here because in the WINS50 dataset it's on Hlev not Slev
  if('synturb' in input):
    tke = data['tke']
  ...
  if('synturb' in input):
    ...
    data  = data.assign({'ustar': ustar, 'vstar': vstar, 'wthls': wthls, 'zi': zi, 'tke': tke})

    # Organize data, rename variables and drop non DALES prognostic variables
  data = data.rename({'ua': 'u', 'va': 'v'})\
    .drop(['ta','p','clw','hus'])\
